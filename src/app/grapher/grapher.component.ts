...
function setupGraph(baseSvg: SVGElement) {
  ...
  const dragg = d3.drag<Element, any>();
  // Selection<BaseType, CircleReflexiveDatum, BaseType, unknown>
  // .call(func: (selection: d3.Selection<d3.BaseType, CircleReflexiveDatum, d3.BaseType, unknown>
  // DragBehavior<GElement extends DraggedElementBaseType, Datum, Subject>
  const drawOnDrag = dragg
  ...
  let path: d3.Selection<d3.BaseType, ForceFlowDatum, d3.BaseType, unknown> =
    svg.append("svg:g").selectAll("path");
  let circle: d3.Selection<
    d3.BaseType,
    CircleReflexiveDatum,
    d3.BaseType,
    unknown
  ...
  function draw() {
    path.attr("d", (a): string => {
      var d: ForceFlowDatum;
  ...
  function restart() {
    ...
    const circles =
      circle.selectAll<d3.BaseType, CircleReflexiveDatum>("circle");
    circles
      .style("fill", (d) =>
        d === selectedNode
  ...
  function keydown() {
    ...
    if (d3.event.keyCode === 17) {
      ...
      // d3.DragBehavior<d3.BaseType, d3.SubjectPosition, {} | d3.SubjectPosition>
      // d3.Selection<d3.BaseType, CircleReflexiveDatum, d3.BaseType, unknown>
      circle.call(drawOnDrag);
